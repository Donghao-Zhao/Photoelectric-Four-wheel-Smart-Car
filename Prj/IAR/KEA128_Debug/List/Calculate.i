...
void AngleCalculate(void)
{
 ...
 double   fDeltaValue=0;
	...
	g_fGravityAngle = (Angle+100- 14500) * 1;  
	g_fGyroscopeAngleSpeed = (gyro_z - 0 ) * 2.15;   

	g_fCarAngle = fGyroscopeAngleIntegral;
	fDeltaValue =(g_fGravityAngle - g_fCarAngle) / 2;      


	fGyroscopeAngleIntegral +=(g_fGyroscopeAngleSpeed + fDeltaValue) / 100;
...
float AngleControl(void) {
        ...
        float   ANGLE_CONTROL_D ;
            ...
            fValue = (0 - g_fCarAngle) * ANGLE_CONTROL_P +                     
	      (0 - g_fGyroscopeAngleSpeed) * ANGLE_CONTROL_D;
...
float SpeedPIDL(int collect_speed,int AmSpeed)
 {
     ...
     SpeedDifferenceL = AmSpeed - collect_speed  ;
         ...
         new_speedL+=SpeedKPL*(SpeedDifferenceL-lastspeeddifferenceL)+SpeedPWMKIL;
...
float SpeedPIDR(int  collect_speed,int AmSpeed)
{
     ...
     SpeedDifferenceR = AmSpeed - collect_speed  ;
         ...
         new_speedR+=SpeedKPR*(SpeedDifferenceR-lastspeeddifferenceR)+SpeedPWMKIR;
...
int MotorSpeedOut(float anglePWM ,float speedPWML,float speedPWMR,float dirPWM )
 ...
 if(Speed_R < -limit) Speed_R = -limit;
   ...
   if(Speed_L > 0) 
    {
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH2, 0);
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH3, Speed_L);
    }
     else
    {
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH2, -Speed_L);
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH3, 0);
    }
     ...
     if(Speed_R > 0)
     {
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH0,Speed_R);
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH1, 0);       
     }
     else
     {
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH0,0);
       ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH1, -Speed_R);
     }
...
short SpeedPWMOut(short NewspeedPWM ,short LastspeedPWM,uint8_t PeriodCount)
{
    ...
    SpeedOUT = speedPWMfav *(PeriodCount)/100 + LastspeedPWM ;
...
short AAangPWMOut(short NewAangPWM ,short LastAangPWM,uint8_t PeriodCount)
{
    ...
    AangOUT = AangPWMfav *(PeriodCount)/(5) + LastAangPWM ;
