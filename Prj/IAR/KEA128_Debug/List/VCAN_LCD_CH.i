...
void LCD_Str_CH(Site_t site, const uint8 *str  , uint16 Color , uint16 bkColor) 		
{
    while(*str != '\0')
    {
        if(site.x > (ST7735S_get_w() - 16))
        {
...
void LCD_Str_ENCH(Site_t site, const uint8 *str  , uint16 Color , uint16 bkColor) 		
{
    while(*str != '\0')
    {
        if( *str < 0x80)       
