...
PTXn_e KEY_PTxn[KEY_MAX] = {PTD0, PTD1, PTD2, PTD3, PTE4, PTD4};
...
void    key_init(KEY_e key)
{
    if(key < KEY_MAX)
    {
        ...
        port_pull(KEY_PTxn[key], PULLUP_ENBLE);         
    }
    else
    {
        key = KEY_MAX;
...
void key_IRQHandler(void)
{
    ...
    for(keynum = (KEY_e)0 ; keynum < KEY_MAX; keynum ++)    
    {
        if(key_get(keynum) == KEY_DOWN)                     
        {
            ...
            else if(keytime[keynum] <= 50)       
            {
                continue;                                   
            }
            else if(keytime[keynum]  == 50 + 1)  
            {
