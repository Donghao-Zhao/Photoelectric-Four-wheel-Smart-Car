...
const uint32 nrf_com_size[COM_MAX] = {0 , TSL1401_MAX *128 , 8, 0};

uint32 nrf_com_totalsize[COM_MAX];                                                                  
...
void nrf_msg_init()
{
    ...
    while(i--)
    {
        nrf_com_totalsize[i] = (( nrf_com_size[i] + 2 * 2 + 32 - 1 ) / 32)  
                               *32;                                                      
    }
...
{
    if (!(com < COM_MAX)) assert_failed("C:\\Users\\ZDH\\Desktop\\2018.6.20.21.23\\2018.6.20.21.23\\Board\\src\\VCAN_NRF24L0_MSG.c", 37);          

    
    sendbuf[0] =  com;
    sendbuf[1] = ~com;
    sendbuf[nrf_com_size[com] + 2 * 2 - 2] = ~com;
    sendbuf[nrf_com_size[com] + 2 * 2 - 1] = com;

    while( !nrf_tx(sendbuf , nrf_com_size[com] + 2 * 2)); 
...
RE_LOOP:
    ...
    if(((uint8)comtmp < (uint8)COM_MAX) && (buftemp[1] ==  (uint8)~comtmp) && (comtmp != COM_RETRAN) )
    {
        ...
        totallen = nrf_com_totalsize[comtmp];   
        ...
        if(totallen > relen )                   
        {
            ...
            while( !nrf_rx_fifo_check(nrf_com_size[comtmp] + 2 * 2 - relen, &tmpcheck)  );  
        ...
        if(
            (rebuf[nrf_com_size[comtmp] + 2 * 2 - 2] ==   (uint8)~comtmp)
            &&  (rebuf[nrf_com_size[comtmp] + 2 * 2 - 1] ==   (uint8) comtmp)   )
        {
            *com = comtmp;                          
            ...
            {
            case COM_VAR:
                last_tab = *((uint32 *)&rebuf[2]);                                    
                if(last_tab < VAR_MAX)
                {
                    save_var((var_tab_e)last_tab, *((uint32 *)&rebuf[2 + sizeof(uint32)]));          
                    var_display(last_tab);                                                  
                }
                else
                {
