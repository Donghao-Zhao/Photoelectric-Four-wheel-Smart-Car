...
void uart_init (UARTn_e uratn, uint32 baud)
{
...
#if (USE_UART0_MODE == 0)    //条件编译
        BIT_CLEAN(SIM_PINSEL0, SIM_PINSEL_UART0PS_SHIFT) ;
#elif (USE_UART0_MODE == 1)//，，，，uart 引脚方案
        port_pull(PTA3, ENABLE);//使能
    ...
    //计算波特率
    sysclk = bus_clk_khz * 1000;                                    //bus时钟，单位变成hz

    //UART 波特率 = UART 模块时钟 / (16 × (SBR[12:0] ))？？//////这里是13位的波特率设置、/UART 波特率 = UART 模块时钟 / (16 × (SBR）
    sbr = (uint16)((sysclk / 16 + baud / 2 ) / (baud ));                 //  baud/2  的作用是没微调寄存器，误差较大，只能四舍五入取最接近的
    if(sbr > 0x1FFF)sbr = 0x1FFF;                                       //SBR 是 13bit，最大为 0x1FFF/////SBR是模数分频因子
